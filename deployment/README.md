...
| Fabric bootstrap notebook + daily schedule | **Full** | Created or definition-updated; an existing schedule is preserved. The Setup Center publishes it from the definition shipped in the solution, immediately runs it to register exported event folders and only the configured baseline Dataverse tables, and waits for completion. Large Dataverse links are not copied table by table; later segment dependencies are added on demand. |
| Fabric Dataverse mirror lakehouse | **Detect only** | Created by the Dataverse "Link to Microsoft Fabric" feature. Setup verifies `contact` and reuses its matching target metadata. A non-schema Dataverse Link Lakehouse remains the source and is never mistaken for the query-serving Lakehouse. |
...
| Query-facing Dataverse shortcuts | **Create, validate, or repair** | Setup and runtime create `Tables/dataverse/*` from the matching source table's exact Dataverse target metadata in the schema-enabled Serving Lakehouse. Existing shortcuts with stale targets are repaired automatically. |
