  ...
  return {
    ...
    "metadata": {
      "_generator": {
        ...
        "templateHash": "9992665777660407798"
      }
    },
    "parameters": {
      "location": {
      ...
      "fabricCapacityResourceId": {
        "type": "string",
        "metadata": {
          "description": "Azure resource ID of the Fabric capacity that hosts the workspace."
        }
      },
      ...
      },
      {
        ...
        "properties": {
          ...
          "siteConfig": {
            ...
            "appSettings": "[concat(createArray(createObject('name', 'APPLICATIONINSIGHTS_CONNECTION_STRING', 'value', reference(resourceId('Microsoft.Insights/components', format('{0}-insights', parameters('webAppName'))), '2020-02-02').ConnectionString), createObject('name', 'ApplicationInsightsAgent_EXTENSION_VERSION', 'value', '~3'), createObject('name', 'ASPNETCORE_ENVIRONMENT', 'value', 'Production'), createObject('name', 'BEHAVIORAL_API_KEY', 'value', parameters('behavioralApiKey')), createObject('name', 'BEHAVIORAL_API_KEY_PREVIOUS', 'value', parameters('behavioralApiKeyPrevious')), createObject('name', 'FABRIC_SQL_SERVER', 'value', parameters('fabricSqlServer')), createObject('name', 'FABRIC_SQL_DATABASE', 'value', parameters('fabricSqlDatabase')), createObject('name', 'FABRIC_CATALOG_CACHE_MINUTES', 'value', '60'), createObject('name', 'FABRIC_WARMUP_INTERVAL_MINUTES', 'value', '4'), createObject('name', 'FABRIC_CAPACITY_WARMUP_SECONDS', 'value', '25'), createObject('name', 'FABRIC_WORKSPACE_ID', 'value', parameters('fabricWorkspaceId')), createObject('name', 'FABRIC_SERVING_LAKEHOUSE_ID', 'value', parameters('fabricServingLakehouseId')), createObject('name', 'FABRIC_CAPACITY_RESOURCE_ID', 'value', parameters('fabricCapacityResourceId')), createObject('name', 'FABRIC_DATAVERSE_LAKEHOUSE_ID', 'value', parameters('fabricDataverseLakehouseId')), createObject('name', 'FABRIC_DATAVERSE_CONNECTION_ID', 'value', parameters('fabricDataverseConnectionId')), createObject('name', 'FABRIC_DATAVERSE_DELTA_FOLDER', 'value', parameters('fabricDataverseDeltaFolder')), createObject('name', 'DATAVERSE_ENVIRONMENT_URL', 'value', parameters('dataverseEnvironmentUrl')), createObject('name', 'SEGMENT_PREVIEW_REQUIRED_TABLES', 'value', parameters('requiredDataverseTables'))), if(variables('deployPackage'), createArray(createObject('name', 'WEBSITE_RUN_FROM_PACKAGE', 'value', variables('packageBlobUrl')), createObject('name', 'WEBSITE_RUN_FROM_PACKAGE_BLOB_MI_RESOURCE_ID', 'value', 'SystemAssigned'), createObject('name', 'SEGMENT_PREVIEW_PACKAGE_VERSION', 'value', parameters('apiPackageVersion')), createObject('name', 'SEGMENT_PREVIEW_PACKAGE_SHA256', 'value', toLower(parameters('apiPackageSha256')))), createArray()))]"
          }
        },
        "dependsOn": [
          "[resourceId('Microsoft.Insights/components', format('{0}-insights', parameters('webAppName')))]",
